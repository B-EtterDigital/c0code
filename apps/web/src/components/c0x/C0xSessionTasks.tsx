      ...
      {remaining.length > 0 ? (
            ...
            >
              {task.status === "inProgress" ? (
                <LoaderCircleIcon
                  ...
                  className="mt-0.5 size-3 shrink-0 text-blue-600 dark:text-blue-400"
                />
              ) : (
                <CircleIcon
                  aria-label="Pending"
                  className="mt-0.5 size-3 shrink-0 text-zinc-500 dark:text-zinc-400"
                />
              )}
        ...
        <p className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
          {steps.length > 0 ? (
            ...
            "The agent has not shared a task list yet."
          )}
        </p>
      )}
      {remaining.length > 10 ? (
        <div className="mt-3 flex items-center justify-between text-xs">
          <span className="text-zinc-500 dark:text-zinc-400">
            Showing {Math.min(limit, remaining.length)} of {remaining.length}
          </span>
          {limit < 50 && limit < remaining.length ? (
            <button
              type="button"
              className="rounded px-2 py-1 hover:bg-zinc-100 dark:hover:bg-zinc-700"
              onClick={() => setLimit((value) => Math.min(50, value + 10))}
            >
              Show {Math.min(10, remaining.length - limit)} more
            </button>
          ) : null}
          {limit > 10 ? (
            <button
