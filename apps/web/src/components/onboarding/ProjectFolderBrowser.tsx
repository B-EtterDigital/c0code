...
export function ProjectFolderBrowser({
  ...
  const entries =
    query.data?.entries.filter((entry) => showHidden || !entry.name.startsWith(".")) ?? [];
  const parent =
    current && /^[A-Za-z]:[\\/]?$/.test(current)
      ? current
  ...
  return (
    <div className="my-3">
      <Button variant="outline" size="sm" disabled={disabled} onClick={() => setOpen(!open)}>
        ...
        {open ? "Close folder browser" : label}
      </Button>
      {open ? (
        <div
          className="mt-2 space-y-2 rounded-lg border border-border bg-background p-3"
          aria-label="Project folder browser"
        >
          <div className="flex gap-2">
            <Button
              ...
              disabled={disabled || query.isPending || !current || current === parent}
          ...
          {query.isPending ? (
            ...
            <div className="max-h-40 overflow-y-auto">
              ...
              {entries.length === 0 ? (
                <p className="py-2 text-sm text-muted-foreground">No subfolders</p>
              ) : null}
            </div>
          )}
          <Button
            size="sm"
            disabled={disabled || query.isPending || !!query.error || !current}
            onClick={() => {
              if (current) {
                onSelect(current);
                setOpen(false);
