  ...
  messages: [
    {
      info: {
        role: "user",
        time: { created: 1000 },
        model: { providerID: "openrouter", modelID: "google/gemini-2.5-pro" },
      },
...
describe("OpenCode native import", () => {
  it("retains native session identity, the full OpenRouter route and both sides of the conversation", () => {
    expect(parseOpenCodeImport(exported, "ses_test", "/repo")).toMatchObject({
      _tag: "Importable",
      thread: {
  ...
  it("rejects a different session, a different project and empty history", () => {
    ...
    expect(() => parseOpenCodeImport({ ...exported, messages: [] }, "ses_test", "/repo")).toThrow(
      "no conversation",
    );
  });
});
