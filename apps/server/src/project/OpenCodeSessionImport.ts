...
export function parseOpenCodeImport(
  ...
  if (
    info.id !== sessionId ||
    !text(info.directory) ||
    normalizeProjectPathForComparison(text(info.directory)) !==
  ...
  for (const entry of Array.isArray(exported.messages) ? exported.messages : []) {
    ...
    if (body && (message.role === "user" || message.role === "assistant"))
      messages.push({
        role: message.role,
        text: body,
        createdAt: timestamp(record(message.time).created),
      });
  }
  if (messages.length === 0)
    throw new Error("The selected OpenCode session has no conversation to resume.");
  ...
  return {
    ...
    thread: {
      ...
      title: text(info.title) || "OpenCode session",
      model,
    ...
    source: {
      ...
      providerInstanceId: instanceId,
      providerSessionId: sessionId,
      filePath: `opencode:session:${sessionId}`,
      size: Buffer.byteLength(JSON.stringify(value)),
      mtimeMs: typeof time.updated === "number" ? time.updated : null,
      device: 0,
      inode: null,
      birthtimeMs: null,
    },
...
export const readOpenCodeSession = Effect.fn("readOpenCodeSession")(function* (
  sessionId: string,
  workspaceRoot: string,
) {
  ...
  if (!/^ses_[a-zA-Z0-9]+$/.test(sessionId))
    return yield* Effect.fail(failure("Invalid OpenCode session id."));
  const service = yield* Effect.serviceOption(ServerSettingsService);
  if (Option.isNone(service))
    return yield* Effect.fail(failure("OpenCode settings are unavailable."));
  const settings = yield* service.value.getSettings.pipe(Effect.mapError(failure));
  const instance: ProviderInstanceConfig = settings.providerInstances[
    ProviderInstanceId.make("opencode")
  ] ?? { driver: ProviderDriverKind.make("opencode"), config: settings.providers.opencode };
  if (instance.driver !== "opencode" || !resolveProviderInstanceEnabled(instance))
    return yield* Effect.fail(
      failure("Enable OpenCode in Your Accounts before resuming this session."),
    );
  const config = yield* Schema.decodeUnknownEffect(OpenCodeSettings)(instance.config ?? {}).pipe(
    Effect.mapError(failure),
  );
  if (config.serverUrl.trim())
    return yield* Effect.fail(
      failure("This session belongs to the local OpenCode CLI. Choose its local account in setup."),
    );
  ...
  if (output.code !== 0)
    return yield* Effect.fail(failure(`OpenCode export stopped with exit code ${output.code}.`));
  const value = yield* Schema.decodeEffect(Schema.fromJsonString(Schema.Unknown))(
    output.stdout,
  ).pipe(Effect.mapError(failure));
  return yield* Effect.try({
    try: () => parseOpenCodeImport(value, sessionId, workspaceRoot),
    catch: failure,
  });
});
