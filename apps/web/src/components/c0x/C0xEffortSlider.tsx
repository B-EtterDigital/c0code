...
export function C0xEffortSlider({
  ...
  const index = Math.max(
    0,
    options.findIndex((option) => option.id === effort),
  );
  ...
  const pickFromPointer = React.useCallback(
    (clientX: number) => {
      ...
      moveTo(
        Math.round(
          Math.min(1, Math.max(0, (clientX - rect.left) / rect.width)) * (options.length - 1),
        ),
      );
    },
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverPopup
        side="top"
        align="center"
        className="w-64 rounded-2xl border border-border bg-popover text-popover-foreground"
      ...
      >
        <div data-testid="c0code-effort-slider">
            ...
            <Tooltip>
              <TooltipTrigger render={<span />}>
                <button
                  ...
                  aria-pressed={fastEnabled === true}
                  disabled={disabled || !onToggleFast}
                  onClick={onToggleFast}
                  className={`rounded-md p-1.5 disabled:opacity-30 ${fastEnabled ? "bg-violet-500/15 text-violet-600 dark:text-violet-300" : "text-muted-foreground hover:bg-accent"}`}
                >
                  <ZapIcon className="size-4" />
                </button>
              </TooltipTrigger>
              <TooltipPopup>
                {onToggleFast ? "Fast mode" : "Fast mode is unavailable for this model"}
              </TooltipPopup>
            </Tooltip>
            <button
              type="button"
              ...
              className="flex min-w-0 flex-1 flex-col items-center rounded-lg px-1 py-0.5 hover:bg-accent"
            >
          ...
          <div
            ...
            onKeyDown={(event) => {
              const offset = ["ArrowLeft", "ArrowDown"].includes(event.key)
                ? -1
          ...
          >
            ...
            {options.map((option, stop) => (
              <span
                aria-hidden="true"
                key={option.id}
                className="absolute top-1/2 size-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/65"
                style={{ left: `${options.length > 1 ? (stop / (options.length - 1)) * 100 : 0}%` }}
              />
            ))}
            <span
              aria-hidden="true"
              className="absolute top-1/2 size-3.5 rounded-full bg-white shadow ring-1 ring-violet-300"
