...
it("shows upcoming actions, expands to 50, and updates from agent events", async () => {
  ...
  await act(() => {
    renderer = create(<C0xSessionTasks plan={plan} />);
  });
  ...
  for (let i = 0; i < 4; i++) {
    const more = renderer.root
      ...
      .find((button) => button.children.join("").includes("more"));
  ...
  expect(
    renderer.root
      .findAllByType("button")
      .some((button) => button.children.join("").includes("more")),
  ).toBe(false);
  await act(() =>
    renderer.update(
      <C0xSessionTasks
        plan={{ ...plan, steps: plan.steps.map((step) => ({ ...step, status: "completed" })) }}
      />,
    ),
  );
...
it("does not invent tasks when the agent has not published a plan", async () => {
  await act(() => {
    renderer = create(<C0xSessionTasks plan={null} />);
  });
  expect(renderer.root.findAllByType("li")).toHaveLength(0);
