...
const options = [{ id: 'low', label: 'Low' }, { id: 'high', label: 'High' }, { id: 'xhigh', label: 'Extra high' }];

it('uses the provider stops and forwards reset, fast and model actions', async () => {
  const onChange = vi.fn(), onReset = vi.fn(), onToggleFast = vi.fn(), onOpenModelPicker = vi.fn();
  await act(() => { renderer = create(<C0xEffortSlider options={options} effort="high" onChange={onChange} onReset={onReset} onToggleFast={onToggleFast} onOpenModelPicker={onOpenModelPicker} modelLabel="Configured model" fastEnabled />); });
  ...
  for (const label of ['Fast mode', 'Reset effort', 'Choose model']) await act(() => renderer.root.findByProps({ 'aria-label': label }).props.onClick());
...
it('blocks unsupported and unavailable provider controls without inventing values', async () => {
  ...
  await act(() => { renderer = create(<C0xEffortSlider options={options.slice(0, 1)} effort="low" disabled onChange={onChange} />); });
  await act(() => renderer.root.findByProps({ role: 'slider' }).props.onKeyDown({ key: 'End', preventDefault: vi.fn() }));
