...
import {
  C0xSessionDetailsView,
  C0xSessionSummary,
  type C0xSessionDetailsViewProps,
} from "./C0xSessionDetails";

export function C0xSessionPanel(
  props: C0xSessionDetailsViewProps & {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white text-zinc-900 dark:bg-[#181818] dark:text-zinc-100">
      ...
      {expanded ? (
        ...
        <div className="min-h-0 overflow-y-auto p-4">
          <section className="mb-5 text-xs" aria-label="Environment">
            ...
            {props.branch ? (
              <div className="mt-1 text-zinc-600 dark:text-zinc-400">{props.branch}</div>
            ) : null}
            {props.onOpenGit ? (
              <button
                type="button"
                className="mt-3 rounded-md border border-zinc-200 px-3 py-1.5 dark:border-zinc-700"
          ...
          >
            Sources, outputs and tool usage
          </button>
        </div>
      )}
    </div>
