...
import { isProjectReadFileError, projectFileReadErrorMessage } from "./projectFileReadFailure";
...
function errorMessage(cause: unknown): string | null {
  ...
  if (isProjectReadFileError(cause)) return projectFileReadErrorMessage(cause);
...
export function useProjectFileQuery(
  ...
  // A folder may have a media extension; only the server can identify it.
  const atom = enabled
  ...
  return {
    ...
    failure: readError?.failure ?? null,
    resolvedPath: readError?.resolvedPath ?? null,
    isPending: result.waiting,
    refresh,
  };
}
