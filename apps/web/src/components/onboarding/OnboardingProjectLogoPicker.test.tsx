...
const { asset, canonicalFavicon } = vi.hoisted(() => ({ asset: vi.fn(), canonicalFavicon: vi.fn() }));
...
vi.mock("../settings/ProjectFaviconPickerDialog", () => ({ ProjectFaviconPickerDialog: () => null }));
...
describe("project image choice before import", () => {
  it("previews the selected file without caching a favicon request for an unsaved project", async () => {
    ...
    await act(() => { renderer = create(<OnboardingProjectLogos.Provider value={{ logos: { alpha: { faviconPath: "brand/logo.webp", projectIcon: null } }, setLogo: vi.fn() }}>
      <OnboardingProjectLogoPicker candidate={{ key: "alpha", title: "Alpha", environmentId: EnvironmentId.make("local"), path: "/dev/Alpha" }} />
    </OnboardingProjectLogos.Provider>); });
    expect(asset).toHaveBeenCalledWith("local", expect.objectContaining({ _tag: "media-file", path: "/dev/Alpha/brand/logo.webp" }));
    expect(canonicalFavicon).not.toHaveBeenCalled();
    expect(renderer.root.findByProps({ alt: "Alpha project logo" }).props.src).toBe("http://localhost/asset/signed-image");
  });
});
