...
export function OnboardingAccounts({
  ...
  for (const provider of providers) {
    if (
      effectiveAccounts[provider.instanceId] ||
      !["codex", "claudeAgent", "opencode"].includes(provider.driver)
    )
      continue;
  ...
  const accounts = Object.entries(effectiveAccounts).filter(([, instance]) =>
    ["codex", "claudeAgent", "opencode"].includes(instance.driver),
  );
  const statusKey = providers
    .map((provider) => `${provider.instanceId}:${provider.auth.status}`)
    .join("|");
  ...
  useEffect(() => {
    ...
    setShared(false);
    void window
      .c0codeAccountsBridge!.syncAccounts(
      ...
      .catch((cause) => {
        ...
        if (active)
          setError(
            "Could not share these accounts with C0VIBE. Retry after the computer reconnects.",
          );
      });
    return () => {
      active = false;
  ...
  useEffect(() => {
    if (!local) return;
    void window
      .c0codeAccountsBridge!.openRouterStatus()
      .then((result) => setKeySaved(result.configured))
      .catch((cause) => {
  ...
  const add = async () => {
    ...
    try {
      const id = ProviderInstanceId.make(
        `${driver}_${randomUUID().replaceAll("-", "").slice(0, 12)}`,
      );
  ...
  const saveKey = async (reuse = false) => {
    ...
    try {
      ...
      if (enabled._tag !== "Success" && !isAtomCommandInterrupted(enabled))
  ...
  return (
    <section className="space-y-3" aria-label={`Accounts on ${machineLabel}`}>
      <h3 className="text-sm font-medium">{machineLabel}</h3>
      {local ? (
        <p className="text-xs text-muted-foreground" role="status">
          {shared
            ? "Available to all C0VIBE features that use your CLI accounts."
            : "Sharing accounts with C0VIBE…"}
        </p>
      ) : (
        <p className="text-xs text-muted-foreground">
          These accounts run on {machineLabel}. Their credentials stay on that computer.
        </p>
      )}
      {accounts.map(([id, account]) => {
        ...
        return (
          <div key={id} className="rounded-lg border border-border bg-background p-3">
            <div className="flex items-center gap-3">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">
                  {account.displayName || getDriverOption(account.driver)?.label || id}
                </p>
                <p className="text-xs text-muted-foreground">
            ...
            {open.has(id) && provider
              ? renderTerminal(provider, () => {
                  setOpen((current) => {
                    const next = new Set(current);
                    next.delete(id);
                    return next;
      ...
      <p className="text-xs text-muted-foreground">
        You can leave several sign-in panels open. Each Claude or Codex account has its own login
        folder.
      </p>
      {local ? (
        <div className="space-y-2 rounded-lg border border-border p-3">
          <h4 className="text-sm font-medium">OpenRouter for OpenCode CLI</h4>
          <p className="text-xs text-muted-foreground">
            {keySaved
              ? "An OpenRouter key is available. Use it across C0VIBE, or replace it below."
              : "Add your OpenRouter API key once for OpenCode and C0VIBE’s other AI features."}
          </p>
          {keySaved ? (
            <Button
              size="sm"
          ...
          <div className="flex gap-2">
            <Input
              ...
              disabled={keySaving}
            />
            <Button disabled={!apiKey.trim() || keySaving} onClick={() => void saveKey()}>
              {keySaving ? "Saving…" : "Connect OpenRouter"}
            </Button>
          </div>
        </div>
      ) : null}
