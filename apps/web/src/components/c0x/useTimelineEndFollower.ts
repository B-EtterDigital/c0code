...
export function useTimelineEndFollower(listRef: { current: LegendListRef | null }, owner: string | undefined) {
  ...
  if (!controller.current) controller.current = createTimelineEndFollower({
    read: () => {
