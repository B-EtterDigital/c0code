...
export function GuidedComputerConnection({
  pairing,
  disabled,
  onBusyChange,
  onConnected,
    ...
    try {
      ...
      if (!target) {
        ...
        if (!value || /[\s\0]/.test(value))
          throw new Error("Enter the computer’s SSH name or user@hostname.");
        ...
        target = {
          ...
          username: user || resolved?.username || null,
          port: portNumber ?? resolved?.port ?? null,
        };
      }
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Could not connect. Check that the computer is online and SSH access is enabled.",
      );
    } finally {
      onBusyChange(false);
    }
      ...
      {method === "pairing" ? (
        ...
        >
          <p className="text-sm text-muted-foreground">
            Choose a saved computer or enter its SSH name. C0CODE uses your existing SSH keys,
            prepares the remote workspace, and connects it automatically.
          </p>
          {discovered.length ? (
            <div
              className="flex max-h-32 flex-wrap gap-2 overflow-auto"
              aria-label="Saved SSH computers"
            >
              {discovered.map((target) => (
                <Button
                  key={`${target.alias}:${target.hostname}`}
                  type="button"
                  variant="outline"
                  size="sm"
                  disabled={disabled}
          ...
          <label className="block text-sm">
            ...
            <Input
              ...
              placeholder="my-server or user@hostname"
              disabled={disabled}
              autoCapitalize="none"
              autoCorrect="off"
              className="mt-1"
          ...
          <Button type="submit" disabled={disabled || !host.trim()}>
            {disabled ? "Connecting and preparing…" : "Connect computer"}
          </Button>
        </form>
      )}
    </div>
