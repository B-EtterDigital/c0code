...
function DestinationSelect({
  ...
  options,
  disabled,
  emptyLabel = "Use current destination",
  onChange,
}: {
  label: string;
  value: string;
...
export function C0xComposerMediaControls({
  state,
  onAction,
  onPreferencesChange,
}: {
    ...
    >
      {state.recording && state.waveform.length > 0 ? (
        ...
        >
          {state.waveform.slice(-32).map((level, index, samples) => (
            <span
              key={index}
              className="h-5 w-0.5 shrink-0 rounded-full transition-transform motion-reduce:transition-none"
              style={{
      ...
      <Button
        ...
        onContextMenu={(event) => {
          ...
          openMenu("dictation", event.currentTarget);
        }}
        onKeyDown={(event) => {
      ...
      <Button
        ...
        onContextMenu={(event) => {
          event.preventDefault();
          openMenu("capture", event.currentTarget);
        }}
        onKeyDown={(event) => {
          if (event.key === "ContextMenu" || (event.shiftKey && event.key === "F10")) {
            event.preventDefault();
      ...
      {state.error ? (
        <Tooltip>
          ...
          <TooltipPopup>{state.error}</TooltipPopup>
        </Tooltip>
      ) : null}
      <Popover
        open={menu !== null}
          ...
          <div className="flex flex-col gap-3 p-1">
            ...
            {dictation ? (
              <DestinationSelect
                ...
                value={preferences.pipelineId}
                emptyLabel="Use selected C0DIC pipeline"
                options={state.pipelines}
                disabled={state.recording || state.busy}
              ...
              <DestinationSelect
                ...
                onChange={(captureMode) => {
                  if (captureMode === "region" || captureMode === "window")
                    onPreferencesChange({ captureMode });
                }}
              />
            )}
            ...
            <DestinationSelect
              ...
              onChange={(id) =>
                onPreferencesChange(
                  dictation ? { transcriptionSessionId: id } : { captureSessionId: id },
                )
              }
            />
            {!autosave ? (
              <p className="text-xs text-muted-foreground">
                Keep temporary media until the message is accepted.
              </p>
            ) : null}
            {state.error ? (
