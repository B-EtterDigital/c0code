...
import { OrchestrationMessage, ThreadMessageSentPayload } from "./orchestration.ts";
...
const decodeMessagePayload = Schema.decodeUnknownSync(ThreadMessageSentPayload);
...
describe("OrchestrationMessageContext", () => {
  ...
  it("is optional on messages and preserved by persisted message events", () => {
    const message = {
      id: "m1",
      role: "user",
      text: "hi",
    ...
    expect(payload.role).toBe("reasoning");
    expect(payload.context?.records[0]?.kind).toBe("image");
  });
});
