...
import {
  ...
  OnboardingProjectLogos,
  type OnboardingProjectLogo,
} from "../onboarding/OnboardingProjectLogoPicker";
import { Dialog, DialogPopup, DialogTitle } from "../ui/dialog";
import { Button } from "../ui/button";

/** Edits the canonical project record: sidebar and every split share its logo. */
export function C0xProjectLogoEditor({
  target,
  onClose,
}: {
  ...
  const project = projects.find(
    (entry) =>
      entry.id === target.projectId &&
      (!target.environmentId || entry.environmentId === target.environmentId),
  );
    ...
    >
      <DialogPopup className="max-w-xl">
        ...
        {project ? (
          ...
          <p role="alert">The project is unavailable.</p>
        )}
        {error ? (
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button disabled={!project || !choice || saving} onClick={() => void save()}>
            {saving ? "Saving…" : "Save"}
          </Button>
        </div>
      </DialogPopup>
