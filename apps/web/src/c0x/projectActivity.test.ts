...
test("promoted drafts follow the actual thread on its chosen computer", () => {
  expect(projectActivityIds({ id: "live", environmentId: "remote" }, {
    stable: { threadId: "old", environmentId: "local", promotedTo: { threadId: "live", environmentId: "remote" } },
...
test("unpromoted drafts only report their matching thread", () => {
  expect(projectActivityIds({ id: "live", environmentId: "local" }, {
